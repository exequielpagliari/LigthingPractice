...
void OGLRenderer::draw() {
	...
	if (mRenderData.rdUseChangedShader) {
		...
		int vertexColorLocation = glGetUniformLocation(mShader.GetShaderProgram(), "ourColor");
		glUseProgram(mShader.GetShaderProgram());
		glUniform4f(vertexColorLocation, mRenderData.rShader, mRenderData.gShader, mRenderData.bShader, 1.0f);
		mShader.use();
	...
	int ligthColor = glGetUniformLocation(mGltfShader.GetShaderProgram(), "lightColor");
	glUseProgram(mGltfShader.GetShaderProgram());
	glUniform3f(ligthColor, mRenderData.rShader, mRenderData.gShader, mRenderData.bShader);
	int ligthPosLocation = glGetUniformLocation(mGltfShader.GetShaderProgram(), "lightPos");
	glUseProgram(mGltfShader.GetShaderProgram());
	glUniform3f(ligthPosLocation, mRenderData.xLight, mRenderData.yLight, mRenderData.zLight);
	mGltfShader.use();
	mGltfModel->draw();
